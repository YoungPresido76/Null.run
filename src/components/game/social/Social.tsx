...
function GiftsTab() {
  ...
  async function handleSend() {
    if (!toName.trim()) return setStatus('Enter a username.');
    const n = parseFloat(amount);
    if (!n || n <= 0) return setStatus('Enter a valid amount.');
    if (currency === 'chills'   && state.chills   < n) return setStatus('Not enough Chills.');
    if (currency === 'diamonds' && state.diamonds  < n) return setStatus('Not enough Diamonds.');
    if (!canGift) return setStatus(`Daily gift limit (${DAILY_GIFT_LIMIT}) reached.`);
    ...
    if (res?.ok) {
      ...
      setStatus(`Gift sent to ${toName} ✓`);
      setToName(''); setAmount('');
    } else {
      setStatus(res?.msg ?? 'Failed to send gift.');
    }
    setLoading(false);
  ...
  async function handleCollect() {
    ...
    setInbox(res.msgs ?? []);
    if (res.msgs.length === 0) setStatus('No pending gifts.');
    setLoading(false);
    ...
    <div className="space-y-4">
      ...
      <div className="glass rounded-xl p-4 border border-neon-green/20">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="font-orbitron text-sm text-neon-green font-bold">COLLECT GIFTS</p>
      ...
      <div className="glass rounded-xl p-4 border border-neon-magenta/20">
        <p className="font-orbitron text-sm neon-magenta font-bold mb-1">SEND GIFT</p>
        ...
        <div className="space-y-2">
          ...
          <button onClick={handleSend} disabled={!canGift || loading}
            className={cn(
              'w-full py-3 rounded-xl font-orbitron text-xs font-bold border tracking-wider transition-all active:scale-95',
              canGift
        ...
        {status && <p className="font-mono text-[10px] text-neon-green text-center mt-2">{status}</p>}
      </div>
    </div>
  );
}
...
function FriendsTab() {
  ...
  async function sendRequest() {
    ...
    const snap = await getDocs(q).catch(() => null);
    if (!snap || snap.empty) { setStatus('Player not found.'); setLoading(false); return; }
    const toUid = snap.docs[0].id;
    if (toUid === myUid) { setStatus("Can't add yourself."); setLoading(false); return; }
    ...
    setStatus(`Request sent to ${search.trim()} ✓`);
    setSearch('');
    setLoading(false);
    setTimeout(() => setStatus(''), 4000);
  }
    ...
    <div className="space-y-4">
      ...
      {requests.length > 0 && (
        <div className="glass rounded-xl p-4 border border-yellow-400/20">
          <p className="font-orbitron text-[11px] text-yellow-400 font-bold mb-3">
              ...
              <div key={r.fromUid} className="flex items-center justify-between gap-2">
                ...
                <div className="flex gap-1.5">
                  <button onClick={() => acceptRequest(r.fromUid, r.fromName)}
                    className="px-3 py-1.5 rounded-lg font-orbitron text-[10px] border border-neon-green/40 text-neon-green bg-neon-green/10 hover:bg-neon-green/20 transition-all">
                    ACCEPT
                  </button>
                  <button onClick={() => declineRequest(r.fromUid)}
                    className="px-3 py-1.5 rounded-lg font-orbitron text-[10px] border border-red-500/30 text-red-400 hover:bg-red-500/10 transition-all">
                    DECLINE
                  </button>
                </div>
              </div>
      ...
      <div className="glass rounded-xl p-4 border border-neon-cyan/15">
        <p className="font-orbitron text-sm neon-cyan font-bold mb-3">ADD FRIEND</p>
        <div className="flex gap-2">
        ...
        {status && <p className="font-mono text-[10px] text-neon-green mt-2 text-center">{status}</p>}
      </div>

      {/* Friends list */}
      <div>
        <p className="font-mono text-[9px] text-white/25 tracking-widest mb-2">
...
function RaidsTab() {
  ...
  async function doRaid() {
    if (!target.trim()) return setStatus('Enter a target username.');
    setLoading(true);
    // Find target player
    ...
    if (defUid === auth.currentUser?.uid) { setStatus("Can't raid yourself."); setLoading(false); return; }

    // Check cooldown
    const ok = await canRaid(defUid);
    if (!ok) { setStatus(`${target} was raided recently. Try another target.`); setLoading(false); return; }

    // Check defender shield
    const defData = defDoc.data();
    const defShieldExpiry = defData.shieldExpiry?.toMillis?.() ?? 0;
    if (defShieldExpiry > Date.now()) { setStatus(`${target} is shielded right now.`); setLoading(false); return; }
    ...
    setStatus(won
      ? `🎯 RAID WIN! Stole ${fmt(chillsGained)} Chills${diamondsGained ? ` + ${diamondsGained}💎` : ''}`
      : `🛡️ RAID FAILED — ${target} defended successfully.`
    );
    setTarget('');
    setLoading(false);
  ...
  async function buyShield() {
    if (state.diamonds < shieldCost) return setStatus('Need 10 💎 for a shield.');
    ...
    await setShield(8);
    setStatus('Shield active for 8 hours ✓');
    setTimeout(() => setStatus(''), 4000);
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="glass rounded-xl p-4 border border-red-500/20">
        ...
        {status && (
          <p className={cn(
            'font-mono text-[11px] mt-2 text-center',
            status.startsWith('🎯') ? 'text-neon-green' : 'text-red-400/80',
          )}>
            {status}
          </p>
        )}
